...
尝试使用 `bpftool btf dump map` 和 `bpftool btf dump prog` 命令，分别查看与 eBPF 映射、eBPF 程序相关联的 BTF 信息。要注意的是，你可以通过多种不同的方式指定单个映射或单个程序。
...
$ bpftool btf list
...
51: name <anon>  size 2289B  prog_ids 44  map_ids 28,25,27
```
